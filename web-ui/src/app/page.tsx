...
import {
  Wind, Navigation, TrendingUp, Activity, MapPin, Clock,
  Zap, Gauge, RefreshCw, Compass, Waves, AlertCircle,
  CheckCircle2, XCircle, Droplets, Eye, Sun, CloudRain,
} from 'lucide-react';
import {
...
interface PointInfo { id: string; lat: number; lon: number; sector: string; }
...
// --- Point Card ---
function PointCard({ pred, onClick, isSelected }: {
  pred: Prediction; onClick: () => void; isSelected: boolean;
}) {
  ...
  return (
    <button onClick={onClick}
      ...
      <div className="mt-3 flex items-center justify-between text-xs">
        <span style={{ color: confColor(conf) }}>Conf {conf.toFixed(0)}%</span>
...
function TrendChart({ data }: { data: TrendPoint[] }) {
  const chartData = useMemo(() => data.map(d => ({
    ...
    speed: d.wind_speed_kn, gust: d.wind_gust_kn, confidence: d.confidence_pct,
  })), [data]);

  if (!chartData.length) return (
    <div className="flex h-64 items-center justify-center text-muted-foreground">
      <Activity className="mr-2 h-5 w-5" /> Loading...
    </div>
  );
    ...
    <ResponsiveContainer width="100%" height={260}>
      <AreaChart data={chartData} margin={{ top: 5, right: 20, bottom: 5, left: -10 }}>
        ...
        <Tooltip contentStyle={{ borderRadius: '8px', border: '1px solid #e5e7eb', fontSize: '12px' }}
          formatter={(v: number) => [`${v?.toFixed(1)} kn`, '']} />
        <ReferenceLine y={8} stroke="#22c55e" strokeDasharray="3 3"
          label={{ value: 'Sailing', fontSize: 10, fill: '#22c55e' }} />
        <Area type="monotone" dataKey="gust" stroke="#dc2626" strokeWidth={1} fill="url(#gustG)" name="Gust" />
        <Area type="monotone" dataKey="speed" stroke="#2b83ba" strokeWidth={2} fill="url(#speedG)" name="Speed" />
...
export default function LakeWindDashboard() {
  ...
  const fetchWind = useCallback(async (horizon: number) => {
    ...
    } catch (err) { console.error('Failed to fetch wind:', err); }
  }, []);
  ...
  const fetchHealth = useCallback(async () => {
    try {
      ...
      const data = await res.json();
      if (data.status === 'ok') setHealth(data.sources || []);
  ...
  const fetchTrend = useCallback(async (point: string) => {
    try {
      ...
      const data = await res.json();
      if (data.status === 'ok') setTrendData(data.data || []);
  ...
  const fetchPoints = useCallback(async () => {
    try {
      ...
      const data = await res.json();
      if (data.status === 'ok') setPoints(data.points || []);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!cancelled) { setLoading(false); setLastUpdate(new Date()); }
    })();
    return () => { cancelled = true; };
  }, []);

  // Refetch on horizon change
  useEffect(() => {
    let cancelled = false;
    (async () => { await fetchWind(selectedHorizon); if (!cancelled) setLastUpdate(new Date()); })();
    return () => { cancelled = true; };
  }, [selectedHorizon, fetchWind]);

  // Refetch trend on point change
  useEffect(() => {
    let cancelled = false;
    (async () => { await fetchTrend(selectedPoint); });
    return () => { cancelled = true; };
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      fetchWind(selectedHorizon); fetchHealth(); setLastUpdate(new Date());
    }, 300000);
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Hyperlocal wind forecasting · Lake Como</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => { fetchWind(selectedHorizon); fetchHealth(); setLastUpdate(new Date()); }}
      ...
      <main className="mx-auto max-w-7xl px-4 py-6">
        ...
        {selectedPred && (
          <section className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-4">
            ...
            <StatCard icon={Zap} label="Best Spot"
        ...
        <section className="mb-6">
          <div className="flex items-center gap-2 overflow-x-auto pb-2">
            {HORIZONS.map(h => (
              <button key={h.hours} onClick={() => setSelectedHorizon(h.hours)}
                  ...
                  {sectorPoints.map(pt => {
                    ...
                    if (!pred) return (
                      <div key={pt.id} className="rounded-xl border border-dashed p-4 text-center text-sm text-muted-foreground">
                        {pt.id.replace(/_/g, ' ')}<div className="mt-1 text-xs">No data</div>
                      </div>
                    );
                    return <PointCard key={pt.id} pred={pred} onClick={() => setSelectedPoint(pt.id)}
              ...
              <div className="rounded-xl border bg-card p-5 shadow-sm">
                ...
                <div className="mt-4">
                  <div className="mb-2 flex items-center gap-2 text-xs font-medium text-muted-foreground">
                    <TrendingUp className="h-3.5 w-3.5" /> WIND TREND (24H)
                  </div>
        ...
        <footer className="mt-12 border-t pt-6 text-center text-xs text-muted-foreground">
          <p>LakeWind AI · MOS bias-corrected · {points.length} virtual points</p>
          <p className="mt-1">
            Model: {selectedPred?.model_version ?? '—'} ·
            Generated: {selectedPred ? new Date(selectedPred.generated_at).toLocaleString('en-US') : '—'}
          </p>
        </footer>
      </main>
    </div>
