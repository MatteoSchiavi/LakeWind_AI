...
def init_db() -> None:
    ...
    try:
        ...
        console.print("[cyan]Checking for data gaps (auto-recovery)...[/cyan]")
        result = recover()
        if result.get("any_recovered"):
            fc = result.get("recovery", {}).get("forecasts", {})
            era5 = result.get("recovery", {}).get("era5", {})
            console.print(f"[green]  Recovered {fc.get('rows_inserted', 0)} forecast rows[/green]")
            console.print(f"[green]  Recovered {era5.get('rows_inserted', 0)} ERA5 rows[/green]")
        else:
            console.print("[green]  No gaps detected — data is current.[/green]")
    except Exception as exc:
        console.print(f"[yellow]Auto-recovery skipped: {exc}[/yellow]")
