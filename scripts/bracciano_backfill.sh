#!/bin/bash
# bracciano_backfill.sh — quota-aware backfill of the 4 Lake Bracciano spots
# (Phase 6, 4th lake). Same recipe as spot_backfill.sh:
#   1. ERA5 reanalysis ground truth (4 pts)
#   2. Historical NWP forecasts, all configured models (4 pts)
# Upsert-safe: bulk_insert_forecast_runs uses ON CONFLICT DO UPDATE, so
# re-runs dedup. Resumable: just re-run after a quota stop.
set -u
cd "$(dirname "$0")/.."

LOG=data/cache/backfill_bracciano.log
mkdir -p data/cache
POINTS="trevignano,bracciano_city,vigna_di_valle,anguillara"
START=2025-09-23
END=2026-09-22

echo "=== bracciano_backfill start $(date -u +%FT%TZ) ===" | tee -a "$LOG"

# --- 1. wait for quota reset: probe a minimal historical-forecast call ---
probe() {
  python3 - <<'PY'
import requests
p = {"latitude": 42.10, "longitude": 12.22, "start_date": "2026-08-01",
...
print("forecast rows per Bracciano point:")
for row in con.execute("""
  SELECT point_id, count(*), count(DISTINCT model_name), min(valid_time), max(valid_time)
  FROM forecast_runs
  WHERE point_id IN ('trevignano','bracciano_city','vigna_di_valle','anguillara')
  GROUP BY 1 ORDER BY 1""").fetchall():
    print(f"  {row[0]:16s} {row[1]:7d} rows {row[2]} models {str(row[3])[:10]} .. {str(row[4])[:10]}")
n = con.execute("SELECT count(*) FROM observations WHERE source='era5_reanalysis'").fetchone()[0]
print(f"  total era5 obs now: {n}")
PY
echo "=== bracciano_backfill done $(date -u +%FT%TZ) ===" | tee -a "$LOG"
