...
def register_v2_commands(app: typer.Typer) -> None:
    ...
    # --- V6.2: Two-phase training (windmojo-inspired) ---

    @app.command("feature-discovery")
    def feature_discovery_cmd(
        days: int = typer.Option(60, help="Training window"),
        top_n: int = typer.Option(50, help="Number of top features to select"),
    ) -> None:
        """V6.2: Phase 1 — feature discovery. Finds the top-N most important features.

        Inspired by windmojo's two-phase training:
        1. Train XGBoost with ALL features
        2. Extract feature importance
        3. Select top features for production model
        """
        logging.basicConfig(level=logging.INFO,
                            format="%(asctime)s [%(levelname)s] %(name)s: %(message)s")
        from datetime import datetime as dt, timedelta
        ...
        if top_features:
            console.print(f"\n[green]Top {len(top_features)} features:[/green]")
    ...
    # --- V6.2: Validation diagrams ---

    @app.command("validation-diagram")
    def validation_diagram_cmd(
        point: str = typer.Option("mid_channel", help="Point to validate"),
        days: int = typer.Option(30, help="Validation window"),
        output: str = typer.Option("data/validation_diagram.png", help="Output PNG path"),
    ) -> None:
        """V6.2: Generate validation diagram (model vs baseline comparison).

        Creates a 4-panel PNG:
        ...
        if vp is None:
            console.print(f"[red]Unknown point: {point}[/red]")
            raise typer.Exit(1)
        obs = access.fetch_latest_observation_near(vp.lat, vp.lon, now, max_age_minutes=days * 24 * 60)

        console.print(f"Generating validation diagram for {point} ({len(preds)} preds, {len(obs)} obs)...")
        png = generate_validation_diagram(preds, obs, point)
        if png:
            from pathlib import Path
            Path(output).parent.mkdir(parents=True, exist_ok=True)
            Path(output).write_bytes(png)
            console.print(f"[green]Saved to {output} ({len(png)} bytes)[/green]")
        else:
            console.print("[red]Not enough matched data for validation diagram.[/red]")
