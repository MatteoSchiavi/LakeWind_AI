...
def _check_api_reachable(url: str, timeout: int = 5) -> bool:
    """Check if an API endpoint is reachable before attempting recovery."""
    try:
        resp = requests.head(url if "/v1/" not in url else url.split("/v1/")[0], timeout=timeout)
...
def recover(
    *,
    ...
    skip_fc = not _check_api_reachable(load_settings().open_meteo.historical_forecast_url)
    if skip_fc:
        logger.info("Historical forecast API unreachable — skipping forecast recovery")
        summary["recovery"]["forecasts"] = {"skipped": "API unreachable"}
    elif fc_gap["needs_recovery"] or force_full:
    ...
    skip_era5 = not _check_api_reachable(load_settings().open_meteo.historical_url)
    if skip_era5:
        logger.info("ERA5 archive API unreachable — skipping observation recovery")
        summary["recovery"]["era5"] = {"skipped": "API unreachable"}
    elif obs_gap["needs_recovery"] or force_full:
